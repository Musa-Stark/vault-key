  ...
  return (
    <div className="vault-card group">
      ...
      <div className="flex items-center gap-2 bg-input rounded-lg px-3 py-2">
        ...
        <button onClick={() => setVisible(!visible)} className="vault-btn-icon p-1">
      ...
      <div className="flex items-center gap-1 mt-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        <button onClick={() => onEdit(id)} className="vault-btn-ghost text-xs flex items-center gap-1">
          <Pencil className="w-3 h-3" /> Edit
        </button>
        <button onClick={() => onDelete(id)} className="vault-btn-ghost text-xs flex items-center gap-1 hover:!text-destructive">
