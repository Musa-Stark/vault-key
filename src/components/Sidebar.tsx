...
const Sidebar = ({ activeView, onNavigate, activeCategory, onCategoryChange }: SidebarProps) => {
      ...
      <nav className="flex-1 p-3 space-y-1">
        {navItems.map((item) => (
          <button
            ...
            className={`vault-sidebar-item w-full ${activeView === item.id ? "active" : ""}`}
          >
          ...
          <div className="mt-6">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`vault-sidebar-item w-full text-sm ${activeCategory === cat ? "active" : ""}`}
              >
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 vault-btn-icon bg-card border border-border"
      >
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-black/50" onClick={() => setMobileOpen(false)}>
          <div
            className="w-64 h-full bg-surface border-r border-border"
